...
class VncApi(VncApiClientGen):
    ...
    _DEFAULT_SECURE_AUTHN_PROTOCOL = "https"
    _DEFAULT_CONTRAIL_API_PROTOCOL = "http"
    _DEFAULT_CONTRAIL_API_SECURE_PROTOCOL = "https"
    ...
    def __init__(self, username=None, password=None, tenant_name=None,
                 ...
                 auth_protocol = None, auth_url=None, api_auth_protocol=_DEFAULT_CONTRAIL_API_PROTOCOL):
    ...
    def _authenticate(self, response=None, headers=None):
        ...
        url = "%s://%s:%s%s" % (self._authn_protocol, self._authn_server, self._authn_port,
                                  self._authn_url)
        try:
            response = requests.post(url, data=self._authn_body, verify=False,
    ...
    def _http_get(self, uri, headers=None, query_params=None):
        if ((self._api_auth_protocol == VncApi._DEFAULT_CONTRAIL_API_SECURE_PROTOCOL) and
           (self._authn_protocol == VncApi._DEFAULT_SECURE_AUTHN_PROTOCOL)):
             url = "https://%s:%s%s" \

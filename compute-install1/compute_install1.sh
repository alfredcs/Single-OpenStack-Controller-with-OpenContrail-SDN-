...
Example:
	compute_install.sh [-L] -o openstack_controller [-c repo_servers_seperated_by_comma ]
...
OPTIONS:
  ...
  -r -- Repo server seperated by comma
...
while getopts "ho:r:LvVD" OPTION; do
case "$OPTION" in
h)
        usage
        exit 0
...
##
[[ ${openstack_controller_name} ]] && openstack_controller=`grep "${openstack_controller_name}\s" /etc/hosts |grep -v ^#|head -1|awk '{print $1}'|head -1`
...
yum clean all
[[ `rpm -qa | grep contrail-openstack-vrouter|wc -l` -lt 1 ]] && { yum -y --disablerepo=* --enablerepo=${REPO_SERVERS} install glusterfs-api glusterfs-libs; yum -y --disablerepo=* --enablerepo=${REPO_SERVERS} install contrail-openstack-vrouter contrail-vrouter abrt pyhton-thrift contrail-nova-vif contrail-setup contrail-nodemgr contrail-vrouter-init python-pycrypto; }
[[ $? -ne 0 ]] && exit 1
...
yum clean all
yum -y --disablerepo=* --enablerepo=${REPO_SERVERS} install openstack-nova-compute python-neutronclient openstack-nova-common python-nova python-novaclient
[[ $? -ne 0 ]] && exit 1
